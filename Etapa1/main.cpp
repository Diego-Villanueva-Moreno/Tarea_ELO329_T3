...
Territory* cargarConfiguracion(const QString &rutaArchivo) {
    ...
    for (int i = 0; i < numPersonas; ++i) {
        ...
        // --- CLAVE: Consumir el salto de línea restante tras el último double ---
        in.readLine();

        // Saltar las líneas de los EloTelTags de forma exacta
        for (int t = 0; t < numTags; ++t) {
            in.readLine();
        }

        // Si tiene tablet asociada (1), saltar también su línea
        if (tieneTablet == 1) {
            in.readLine();
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    view.setWindowTitle("EloTelTag Simulation - Stage 1");
    view.show();
